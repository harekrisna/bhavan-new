...
final class MusicInterpretPresenter extends BasePresenter {        
    ...
    protected function startup()  {
        parent::startup();
   		$this->model = $this->musicInterpret;
        ...
        if(file_exists($interpret_folder)) {
			...
			try {
	        	...
	        	if($values['image']->isOK()) {
					...
					$image->save("./images/music_interpret_avatars/".$preview_file_name, 80);
					chmod("./images/music_interpret_avatars/".$preview_file_name, 0777);
		...
		try	{
		    ...
		    if($values['image']->isOK()) {
   				if($this->record->image)
					@unlink("./images/music_interpret_avatars/".$this->record->image);
